...
void CGameFramework::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		...
		if ((dwDirection != 0) || (dwVerticalDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
		{
			...
			if (dwDirection)
			{
				float fAcceleration = (bLevel2 ? 230.0f : 720.0f) * m_GameTimer.GetTimeElapsed();
				m_pPlayer->Move(dwDirection, fAcceleration, true);
			}
...
void CGameFramework::FrameAdvance()
{
	...
	m_pd3dCommandList->ClearDepthStencilView(d3dDsvCPUDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);
#endif
	if (m_pPlayer && (!m_pScene || m_pScene->IsLevelScene()))
	{
		m_pPlayer->SetLevel2ModelEnabled(m_pScene && m_pScene->IsLevel2Scene());
